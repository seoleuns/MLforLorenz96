...
class DiffSTG(nn.Module):
    ...
    def p_sample_loop(self, c, timestepnum, n_ddim):
        """
        ...
        with torch.no_grad():
            torch.manual_seed(2025+timestepnum)
    ...
    def p_sample_loop_ddim(self, c, timestepnum, n_ddim):
        x_masked, _, _ = c
        ...
        #### Working Best when it is so like this:
        ...
        xs, x0_preds = generalized_steps(x, seq, n_ddim, self.eps_model, self.beta, c, eta=1)
        return xs, x0_preds
    ...
    def evaluate(self, input, timestepnum, n_ddim, n_samples=2):
        x_masked, _, _ = input
        ...
        if self.sample_strategy == 'ddim_multi':
            ...
            xs, x0_preds = self.p_sample_loop_ddim((x_masked, _, _), timestepnum, n_ddim)
            x = xs[-1]
        ...
        if self.sample_strategy == 'ddpm':
            x_masked = x_masked.reshape(-1, F, V, T)#.to(self.config.device)
            x = self.p_sample_loop((x_masked, _, _), timestepnum, n_ddim)
            x = x.reshape(B, n_samples, F, V, T)
            return x  # (B, n_samples, F, V, T)
        else:
            raise  NotImplementedError

    def forward(self, input, timestepnum, n_ddim, n_samples = 1):

        return self.evaluate(input, timestepnum, n_ddim, n_samples)
...
def generalized_steps(x, seq, n_ddim, model, b, c, **kwargs):
    with torch.no_grad():
        n = x.size(0)
