...
from utils.common_utils import draw_predicted_distribution, dir_check, to_device, ws, unfold_dict, dict_merge, GpuId2CudaId, Logger
...
def default_config(data='AIR_BJ'):
    ...
    if config.data.name == 'LoM':
        ...
        config.data.val_start_idx = int(2010000 * 0.995)
        config.data.test_start_idx = int(2010000 * 0.998)
    ...
    config.epoch = 70  # Number of max training epoch
...
def evals(model, data_loader, epoch, metric, config, clean_data, mode='Test'):
    ...
    if mode == 'test': # save the prediction result to file
        ...
        draw_predicted_distribution(samples, targets, observed_flag, evaluate_flag, config)
