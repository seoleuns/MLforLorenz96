...
def draw_predicted_distribution(samples, target, observed_flag, evaluate_flag, config={}):
    ...
    print("CHECKSAMPLE=", samples.shape)
    B, n_samples, T, V, F = samples.shape
    print('node number:', V)
    # take out the last feature
    all_sample_np = samples[:, :, :, :, 0].cpu().numpy()
    samples = samples[:, :, :, :, 0]
    all_sample_np = np.mean(all_sample_np, axis=1)
    ...
    dataind = config.get('dataind', 0)  # change to visualize a different sample

    plt.rcParams["font.size"] = 16
    fig, axes = config.get('fig_axes', (None, None))
    w,h = 6.8, 1.5*4.2# 6, 4.5
    # nrow, ncol = 9, 4
    nrow, ncol = 1, 3
    if fig is None:
        fig, axes = plt.subplots(nrows=nrow, ncols=ncol, figsize=(ncol * w, nrow * h ), squeeze=False)
        print('create fig')
    ...
    for v_idx, v in enumerate(V_lst):
        df = pd.DataFrame({"x": np.arange(0, T), "val": all_sample_np[dataind, :, v], "y": all_evalpoint_np[dataind, :, v]})
        #df = pd.DataFrame({"x": np.arange(0, T), "val": all_target_np[dataind, :, v], "y": all_evalpoint_np[dataind, :, v]})
        df = df[df.y != 0]
        df2 = pd.DataFrame({"x": np.arange(0, T), "val": all_target_np[dataind, :, v], "y": all_observed_np[dataind, :, v]})
        df2 = df2[df2.y != 0]
        row = v_idx // ncol
        col = v_idx % ncol

        color, linestyle, label, alpha = config.get('color', 'g'), config.get('linestyle', 'solid'), config.get('label', '90% interval'), config.get('alpha', 0.3)
        ...
        axes[row][col].plot(df.x, df.val, color='b', marker='o', linestyle='solid', label='prediction')
        axes[row][col].plot(df2.x, df2.val, color='r', marker='x', linestyle='solid', label='truth')
        axes[row][col].set_title(f'node:{v}')

        plt.setp(axes[row][0], ylabel=f'state')
