# -*- coding: utf-8 -*-
#
# LAFDDIM generated by Seoleun Shin (KRISS), April, 2025
#
#---------------------------------------------------------------------------------
#
import easydict
...
class DiffSTG(nn.Module):
    ...
    def p_sample_loop_ddim(self, c, timestepnum, n_ddim):
        ...
        if skip_type == "uniform":
            ...
            # Define a linear beta schedule for forward diffusion
            seq = (np.linspace(0., np.sqrt(N * 0.8), timesteps) ** 2)
            #seq = (np.linspace(0., np.sqrt(N * 0.8 + np.random.normal(0, 0.05)), timesteps) ** 2)
            #seq = (np.linspace(0., np.sqrt(N * 0.9 + np.random.normal(0, 0.01)), timesteps) ** 2)
            #seq = (np.linspace(0., np.sqrt(N * 0.9), timesteps) ** 2)
        ...
        xsreturn, x0_preds = generalized_steps(x, seq, n_ddim, self.eps_model, self.beta, c, eta=1)
        xsreturn = [tensor.to(device=self.device) for tensor in xsreturn]
        xs = torch.stack(xsreturn, dim=0)

        import matplotlib.pyplot as plt
        datahmm = xs.cpu().detach().numpy()
        randomtime = np.zeros(B)
        AA, BB, CC, DD = B, F, V, T
        multiddpmx = np.zeros((AA, BB, CC, DD))
        print("DATA=", timestepnum)
        #print("DATA=", datahmm.shape)
        #print("DATA=", AA,BB,CC,DD)

        halfnddim = int(n_ddim/2) + 1
        for i in range(B):
            np.random.seed(40003+timestepnum+i)
            randpert = halfnddim*np.random.rand(1)
            indpert = int(randpert)
            multiddpmx[i,:,:,:] =  datahmm[-1-indpert,i,:,:,:]
            randomtime[i]= n_ddim-1-indpert


        x = torch.from_numpy(multiddpmx).float()

        dataalpha = datahmm[:-1,:,:,:,1]  # For T_p=1, and all reverse process 
        dataalpha = dataalpha[:,:,0,20]  # At x=20 
        dataalpha = dataalpha.reshape(n_ddim,B) # the first is reverse steps and the second is n_samples
        markerx = multiddpmx[:,0,20,1]
        markerx = markerx.reshape(B)

        if timestepnum <= 400:
        #if timestepnum >= 5000000000:
            plt.figure(figsize=(12, 8))
            timeseq = np.linspace(0, n_ddim-1, n_ddim)
            #timeseq = np.linspace(1, n_ddim+1, n_ddim)
            plt.plot(timeseq, dataalpha, linewidth=3.0, zorder=1)
            for i in range(B):
               plt.scatter(randomtime[i], markerx[i], color='dimgray', s=150, label='Marker', zorder=2)
            plt.xticks(size = 24)
            plt.yticks(size = 24)
            plt.ylim(-4, 8)
            plt.xlabel('Sequence', fontsize=24)
            plt.ylabel('State Value', fontsize=24)
            plt.title('Trajectory of Samples in Reverse Process', fontsize=24)
            plt.savefig('transitionm_multiddim%s.png'%(timestepnum))
            plt.close()

        #return xsreturn, x0_preds
        return x, x0_preds
    ...
    def evaluate(self, input, timestepnum, n_ddim, n_samples=2):
        ...
        if self.sample_strategy == 'ddim_multi':
            ...
            x = xs.reshape(B, n_samples, F, V, T)
